...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public JwtUserDto loadUserByUsername(String username) {
        ...
        if (loginProperties.isCacheEnable() && redisUtils.hasKey(RedisUserTypeEnum.ADMIN.getType().concat(username))) {
            jwtUserDto = (JwtUserDto) SerializationUtils.deserialize((byte[]) redisUtils.get(RedisUserTypeEnum.ADMIN.getType().concat(username)));
            searchDb = false;
        }
        if (searchDb) {
            UserDTO userDto;
            ...
            if (ObjectUtil.isEmpty(userDto)) {
                ...
                redisUtils.set(RedisUserTypeEnum.ADMIN.getType().concat(username), SerializationUtils.serialize(jwtUserDto));
            }
        }
        return jwtUserDto;
    }
