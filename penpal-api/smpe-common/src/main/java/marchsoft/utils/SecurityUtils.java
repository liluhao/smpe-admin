...
public class SecurityUtils {
    ...
    public static UserDetails getCurrentUser() {
        ...
        if (authentication.getPrincipal() instanceof UserDetails) {
            ...
            // 后台用户登录有nickName，前台没有。可以通过此来判断
            RedisUtils redisUtils = SpringContextHolder.getBean(RedisUtils.class);
            String token = (String) authentication.getCredentials();
            String key = "online-token-" + token;
            Map<String, Object> onlineInfo = BeanUtil.beanToMap(redisUtils.get(key));
            String nickName = (String) onlineInfo.get("nickName");
            UserDetailsService userDetailsService;
            if (ObjectUtil.isNull(nickName)) {
                // 学生流程登录
                userDetailsService = SpringContextHolder.getBean("studentUserDetailsService");
            }else {
                userDetailsService = SpringContextHolder.getBean("userDetailsService");
            }
